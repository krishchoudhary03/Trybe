...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      setError('Please enter your email or College ID.');
      return;
    }
    if (!password) {
    ...
    setError('');
    const derivedName = email.split('@')[0].split('.')[0];
  ...
  const handleForgotPassword = () => {
    if (!email.trim() || !email.includes('@')) {
      showToast('Please enter your university email address first.', 'error');
      return;
    }
  ...
  return (
    <div className="bg-background text-on-background font-body-md min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <main className="relative z-10 w-full max-w-[440px] px-margin-mobile md:px-0">
        ...
        >
          ...
          <header className="flex flex-col items-center mb-xl">
            ...
            <h2 className="font-headline-md text-headline-md text-on-surface mb-base text-center">Welcome back</h2>
            <p className="font-body-sm text-body-sm text-on-surface-variant text-center">Enter your details to access your network.</p>
          </header>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-md">
            ...
            <motion.button 
              ...
              type="button" 
              onClick={() => {
                login('alex@university.edu', 'Alex Morgan');
                navigate('/home');
              }}
            >
          ...
          <footer className="mt-xl text-center">
            <p className="font-body-sm text-body-sm text-on-surface-variant">
              ...
              <Link to="/signup" className="text-primary hover:text-primary-fixed transition-colors font-medium underline underline-offset-4 decoration-primary/30 hover:decoration-primary ml-1">Sign up</Link>
            </p>
          </footer>
        </motion.div>
      </main>
